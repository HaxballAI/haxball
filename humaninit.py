...
def probmove(arr):
    ...
    for i in arr:
        cumprob += i
        probcutoffs.append(cumprob)
    j = 0
    while (probcutoffs[j] < move):
        j += 1
    return j
...
for k in range(length):
    j = counting[k]
    if (j < cutoff*length):
        x_train.append(x_init[j])
        y_train.append(y_init[j])
    else:
        x_test.append(x_init[j])
        y_test.append(y_init[j])

x_train = np.array([np.array(y1) for y1 in x_train])
y_train = np.array([np.array(y2) for y2 in y_train])
x_test = np.array([np.array(y3) for y3 in x_test])
y_test = np.array([np.array(y4) for y4 in y_test])
...
redactivation = np.zeros(100)
blueactivation = np.zeros(100)
totalframes = 0
epsilon = 1
...
def policy(signal):
    ...
    return intermediate**10/np.sum(intermediate**10)

def data(x,y): return games[math.floor(x/50)][0][math.floor(y*len(games[math.floor(x/50)][0]))]

def forward(x):
    a0 = np.tanh(np.dot(network[0], np.append(x, 1)))
    a1 = np.tanh(np.dot(network[1], np.append(a0, 1)))
...
while run:
    ...
    blueinput = np.array([(windowwidth - blues[0].pos[0] - 15)*2 / (windowwidth - 30) - 1, (blues[0].pos[1] - 15)*2 / (windowheight - 30) - 1, (blues[0].pos[0] - reds[0].pos[0]) / (windowwidth - 30), (reds[0].pos[1] - blues[0].pos[1]) / (windowheight - 30), (blues[0].pos[0] - b.pos[0]) / (windowwidth - 25), (b.pos[1] - blues[0].pos[1]) / (windowheight - 25)])

    fps = clock.get_fps()
    #print(frames / timeelapsed)
    #print(redinput)

#    balltoredgoal = np.linalg.norm([pitchcornerx - b.pos[0], 200 - b.pos[1]])
#    balltobluegoal = np.linalg.norm([windowwidth - pitchcornerx - b.pos[0], 200 - b.pos[1]])
#    bluetoball = np.linalg.norm([b.pos[0] - blues[0].pos[0], b.pos[1] - blues[0].pos[1]])
#    redtoball = np.linalg.norm([b.pos[0] - reds[0].pos[0], b.pos[1] - reds[0].pos[1]])
#    blueeval = .95 * (.8 * (redtoball - bluetoball) / 785 + .2 * (balltoredgoal - balltobluegoal) / 640) + .5
#    if frames >1:
#        gamedata[-1].append(blueeval)
#        print([gamedata[-1][-1],blueeval])
    ...
    if keys[pygame.K_a] or 0 <= redchoice[0] <= 5:
        if keys[pygame.K_w] or 4 <= redchoice[0] <= 5:
            reds[0].acc = np.array([-1.0, -1.0]) / (2) ** (1 / 2)
        elif keys[pygame.K_s] or 0 <= redchoice[0] <= 1:
            reds[0].acc = np.array([-1.0, 1.0]) / (2) ** (1 / 2)
        else:
            reds[0].acc = np.array([-1.0, 0.0])

    elif keys[pygame.K_d] or 8 <= redchoice[0] <= 13:
        if keys[pygame.K_w] or 8 <= redchoice[0] <= 9:
            reds[0].acc = np.array([1.0, -1.0]) / (2) ** (1 / 2)
        elif keys[pygame.K_s] or 12 <= redchoice[0] <= 13:
            reds[0].acc = np.array([1.0, 1.0]) / (2) ** (1 / 2)
        else:
            reds[0].acc = np.array([1.0, 0.0])

    elif keys[pygame.K_w] or 6 <= redchoice[0] <= 7:
        reds[0].acc = np.array([0.0, -1.0])

    elif keys[pygame.K_s] or 14 <= redchoice[0] <= 15:
        reds[0].acc = np.array([0.0, 1.0])

    else:
        reds[0].acc = np.array([0.0, 0.0])

    if keys[pygame.K_v] or redchoice[0] % 2 == 0:
        reds[0].kicking = True
    else:
    ...
    if keys[pygame.K_LEFT] or 8 <= bluechoice[0] <= 13:
        if keys[pygame.K_UP] or 8 <= bluechoice[0] <= 9:
            blues[0].acc = np.array([- 1.0, - 1.0]) / (2) ** (1 / 2)
        elif keys[pygame.K_DOWN] or 12 <= bluechoice[0] <= 13:
            blues[0].acc = np.array([- 1.0, 1.0]) / (2) ** (1 / 2)
        else:
            blues[0].acc = np.array([- 1.0, 0.0])

    elif keys[pygame.K_RIGHT] or 0 <= bluechoice[0] <= 5:
        if keys[pygame.K_UP] or 4 <= bluechoice[0] <= 5:
            blues[0].acc = np.array([1.0, - 1.0]) / (2) ** (1 / 2)
        elif keys[pygame.K_DOWN] or 0 <= bluechoice[0] <= 1:
            blues[0].acc = np.array([1.0, 1.0]) / (2) ** (1 / 2)
        else:
            blues[0].acc = np.array([1.0, 0.0])

    elif keys[pygame.K_UP] or 6 <= bluechoice[0] <= 7:
        blues[0].acc = np.array([0.0, -1.0])

    elif keys[pygame.K_DOWN] or 14 <= bluechoice[0] <= 15:
        blues[0].acc = np.array([0.0, 1.0])

    else:
        blues[0].acc = np.array([0.0, 0.0])

    if keys[pygame.K_RCTRL] or bluechoice[0] % 2 == 0:
