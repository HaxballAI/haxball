...
if __name__ == "__main__":
    ...
    if True:
        model = torch.load("models/arun_v5_1.model")
        model_fixed_opponent = torch.load("models/arun_v5_2.model")
        ...
        trainer = FixedTrainSession(model_training=model, model_fixed=model_fixed_opponent, env = lambda: makeEnv(15, False), worker_number=15,\
                                      batch_size=64, learning_rate=3e-4, gamma=1-3e-3, entropy_rate=0.004, is_norming=False)
        for i in range(100):
            print("Step " + str(i), global_timer.getElapsedTime())
            trainer.runStep()
        torch.save(model, "models/arun_v5_3.model")
